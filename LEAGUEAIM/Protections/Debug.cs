	...
	class AntiDebug
	{
		...
		public static bool FindWindowAntiDebug()
		{
			...
			foreach (Process GetWindow in GetProcesses)
			{
				...
				{
					if (GetWindow.MainWindowTitle.ToLower().Contains(BadWindows, StringComparison.CurrentCultureIgnoreCase) && !GetWindow.MainWindowTitle.Contains("tidal", StringComparison.CurrentCultureIgnoreCase))
						return true;
				}
			}
			return false;
		...
		public static bool GetForegroundWindowAntiDebug()
		{
			...
			if (WindowLength != 0)
			{
				...
				_ = GetWindowTextA(HWND, WindowName, WindowLength + 1);
				foreach (string BadWindows in BadWindowNames)
				{
					if (WindowName.ToString().ToLower().Contains(BadWindows, StringComparison.CurrentCultureIgnoreCase) && !WindowName.ToString().ToLower().Contains("tidal", StringComparison.CurrentCultureIgnoreCase))
						return true;
				}
			}
			return false;
